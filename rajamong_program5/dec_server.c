...
{
	// variable to store the decripted characters
	char decripted;
	...
	for (x = 0; message[x] != '\n'; x++) 
	{
		...
		// subtracts the value in the index of key with the value of the index of message
		decripted = message[x] - key[x];

		// if the decripted character is negative
		if (decripted < 0)
		{
			// add 27 so that it is a valid character
			decripted = decripted + 27;
		}

		// if the character stored in the index of the temp array is not 26
		if (decripted != ALPHABETSIZE)
		{
			// adds the decripted character value added by the ascii value for A
			// this newly calculated value is stores in the index of message
			message[x] = decripted + 65;
		}

		// if the character stored in the decripted variable is 26
		else
		{
...
int main(int argc, char* argv[])
{
	...
	// accepts a connection, clocking if one is not available until one connects
	// syntax from the server.c file provided on canvas
	while (1)
	{
		...
		// this means that the child will handle the connection
		if (pid == 0)
		{
			...
			// decrypts the message by calling decrypting function
			decryptmessage(message, start);
